...
function Card(props) {
  return (
    <div className=' h-full w-72  rounded-3xl relative overflow-hidden shrink-0 hover:scale-95 transition-all duration-300'>
        <img src={props.img} className='absolute h-full w-full object-cover opacity-90' alt=''/>
<div className=' h-full absolute top-5 left-5 flex flex-col justify-between w-[90%] ' > 
<div className='h-12 w-12 bg-white rounded-full text-black px-4 py-2 flex items-center justify-center'>{props.id+1}</div>

<div className='pb-12 '>
    <p className='pb-4 text-black [text-shadow:2px_2px_5px_white] font-semibold capitalize'>{props.words}</p>
